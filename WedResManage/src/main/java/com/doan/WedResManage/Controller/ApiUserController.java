...
public class ApiUserController {
    ...
    @PostMapping("/order/request-cancel")
    public ResponseEntity<?> requestCancel(@RequestBody int id) {
        WeddingPartyOrders temp = weddingPartyOrders.findById(id);
        temp.setStatus(5);
        return ResponseEntity.ok(weddingPartyOrders.save(temp));
    }
    @PostMapping("order/update-payment-stt")
    public ResponseEntity<?> updatePayment(@RequestBody PaymentRequest params){
        WeddingPartyOrders wpo=weddingPartyOrders.findById(params.getId());
        wpo.setTypePay(params.getTypePay());
        wpo.setStatus(params.getStatus());
        wpo.setTransId(params.getTransId());
        return ResponseEntity.ok(weddingPartyOrders.save(wpo));
    }
}
